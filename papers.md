...
* Эмбеддинги (в статье) всегда нормализуются *(живут на единичной гиперсфере)*.
...
* В экспериментах авторы используют [ZFNet](https://arxiv.org/abs/1311.2901) и разные вариации [GoogLeNet (Inception v1)](https://arxiv.org/pdf/1409.4842.pdf) и вообще замеряют скорость/память/FLOPS'ы.
...
* В статье рассматривается проблема ре-идентификации: в случае CV появляется новое изображение (другой ракурс, камера, свет и т.п.) и естественно ожидать, что новые эмбеддинги будут обладать теми же свойствами. Однако на практике при использовании триплет-лосса оказывается, что внутриклассовая дисперсия достаточно сильная, в результате чего облака перекрываются, на новых данных эффет усиливается.

<p align="center"><img src="vis/quadruplet_ill.png" width="300"></p>

* Формула для самого лосса приведена на **(3)**:
  1. Сближаем объекты из одного класса
  2. Отдаляем от них другой класс (1 слагаемое)
  3. Делаем межклассовое расстояние больше внутриклассового (2-е слагаемое)
<p align="center"><img src="vis/quadruplet_loss.png" width="500"></p>

* Таким образом, что особенно полезнт в идентификации лиц, итоговые изображения получаются собранными в одну кучку своего кластера с большим межкластерным расстоянием. Второй констрейнт обычно меньше.
* Датасеты для замера: [CUHK03](https://www.ee.cuhk.edu.hk/~xgwang/CUHK_identification.html), [CUHK01](https://www.ee.cuhk.edu.hk/~xgwang/CUHK_identification.html), все результаты в таблице:

<p align="center"><img src="vis/quadruplet_table.png" width="500"></p>

* Экспериментально влияние на внутри- и межкластерный разброс подтвердилось:

<p align="center"><img src="vis/intra_inter_qudruplet.png" width="500"></p>

* Кроме того, авторы предлагают улучшить триплет-лосс: вместо евклидовой нормы на обученные эмбеддинги они обучают саму метрику (отображение в 1/2-мерное пространство), и такой подход работает лучше *(но в этом случае необходимо менять архитектуру сети или сильно достраивать, чтобы подавать на вход сразу 2 картинки — просится аттеншн, но похоже, что авторы использовали конкатенацию и свертку).*